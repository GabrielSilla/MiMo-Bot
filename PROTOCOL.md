...
| `REPORT <buildOk> <buildFail> <commits> <minReuniao> <minMidia> <minJogo> <RATING> <texto>` | O Relatório do dia do Brobot.Sender (ver abaixo) — mesma prioridade/duração/formato atômico do `NOTIFY`/`ACHIEVEMENT`, com seis inteiros e um `<RATING>` (`PESSIMO`, `RUIM`, `QUESTIONAVEL`, `MEDIO`, `BOM`, `EXCELENTE`) antes do texto livre. O Core desenha os sete números como sete linhas separadas, não como frase — é a única notificação com olhos pequenos **no topo** em vez de à esquerda. |
...
- **`REPORT`** (o Relatório do dia do Brobot.Sender, ver `DailyReportTracker.cs`
  ...
  precisa da tela inteira **abaixo** dos olhos para sete linhas empilhadas,
  uma por item, em vez de uma frase só. Chega numa linha própria, atômica
  como `ACHIEVEMENT`, mas com **seis números** antes do texto:
  `REPORT <buildOk> <buildFail> <commits> <minutosReuniao> <minutosMidia>
  <minutosJogo> <RATING> <texto>`. `<commits>` vem do hook de git que roda
  em `post-commit` (`hooks/mimo-git-hook.ps1`/`hooks/git-hooks/post-commit`),
  não de um monitor do Windows como os outros números. `<RATING>` é um de
  `PESSIMO`, `RUIM`, `QUESTIONAVEL`, `MEDIO`, `BOM`, `EXCELENTE` — o veredito
  que `DailyReportScoring.cs` já calculou; o Core não julga o dia, só decide
  como o veredito **aparece** na tela. As sete linhas (`Builds OK: N`,
  `Builds Falha: N`, `Commits: N`, `Reuniao: <fmt>`, `Midia: <fmt>`,
  `Jogo: <fmt>`, `Desempenho: <rótulo>`) são escritas de uma vez, sem efeito
  de máquina de escrever — são dado, não fala, o mesmo motivo pelo qual a
  xícara do `COFFEE` ou o troféu do `ACHIEVEMENT` nunca digitam. Só o texto
  livre no fim da linha (o título que diferencia relatório completo de
  parcial, mais a frase descontraída sorteada por `DailyReportMessages.cs`)
  tipa e quebra linha como em qualquer outra notificação, só que começando
  mais embaixo na tela — depois das sete linhas de números, não na posição
  fixa de sempre.
