...
- **`REPORT`** (o Relatório do dia do Brobot.Sender, ver `DailyReportTracker.cs`
  ...
  precisa da tela inteira **abaixo** dos olhos para linhas empilhadas, uma
  por item, em vez de uma frase só. Chega numa linha própria, atômica como
  `ACHIEVEMENT`, mas com **oito números** antes do texto:
  ...
  (ver `YouTubeTabDetector.cs`), o outro sobre redes sociais com aba focada
  (ver `SocialMediaTabDetector.cs`), e ficam separados por decisão de
  produto, não por limitação técnica. `<RATING>` é um de `PESSIMO`, `RUIM`,
  `QUESTIONAVEL`, `MEDIO`, `BOM`, `EXCELENTE` — o veredito que
  `DailyReportScoring.cs` já calculou; o Core não julga o dia, só decide
  como o veredito **aparece** na tela.

  Nove itens (`Builds OK: N`, `Builds Falha: N`, `Commits: N`,
  `Reuniao: <fmt>`, `Midia: <fmt>`, `Youtube: <fmt>`, `Rede Social: <fmt>`,
  `Jogo: <fmt>`, `Desempenho: <rótulo>`) não cabem numa tela só sem apertar
  demais — uma versão anterior tentou, com olhos espremidos e a frase final
  cabendo em só 2 linhas com quase nenhuma margem. Agora `REPORT` se espalha
  em **duas páginas**, até `NOTIF_REPORT_PAGE_ITEMS` (6) itens por página —
  hoje isso é 6 na primeira e os 3 restantes na segunda, não 6+6, já que
  nove não divide igual — cada uma na tela por `NOTIF_REPORT_PAGE_DURATION_MS`
  (5s — mais curto que os 10s de toda outra notificação, decisão de produto:
  uma página é só pra ler, não tem texto digitando igual a frase final tem),
  então `REPORT` inteiro dura `NOTIF_REPORT_TOTAL_DURATION_MS` (10s no
  total, as duas páginas somadas) — o mesmo tanto de sempre, só que dividido
  em dois. A virada acontece sozinha na metade do caminho (`reportOnSecondPage`,
  comparando o tempo decorrido contra `NOTIF_REPORT_PAGE_DURATION_MS`). Os
  itens de cada página são escritos de uma vez, sem efeito de máquina de
  escrever — são dado, não
  fala, o mesmo motivo pelo qual a xícara do `COFFEE` ou o troféu do
  `ACHIEVEMENT` nunca digitam. Só o texto livre no fim da linha (o título
  que diferencia relatório completo de parcial, mais a frase descontraída
  sorteada por `DailyReportMessages.cs`) tipa e quebra linha como em
  qualquer outra notificação — em **três** linhas de novo (a folga que as
  duas páginas abriram trouxe de volta a terceira linha que a versão de
  página única tinha cortado), começando mais embaixo na segunda página do
  que na primeira, já que a segunda tem menos itens acima dela e sobra mais
  espaço. A frase em si é a mesma nas duas páginas — como ela já termina de
  digitar bem dentro dos primeiros 5s (frases curtas, ~40ms por caractere),
  a segunda página só mostra o que já tinha sido revelado, sem retypar nada.
