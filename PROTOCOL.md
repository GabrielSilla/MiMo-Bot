...
| `REPORT <buildOk> <buildFail> <commits> <minReuniao> <minMidia> <minVideo> <minJogo> <RATING> <texto>` | O Relatório do dia do Brobot.Sender (ver abaixo) — mesma prioridade/duração/formato atômico do `NOTIFY`/`ACHIEVEMENT`, com sete inteiros e um `<RATING>` (`PESSIMO`, `RUIM`, `QUESTIONAVEL`, `MEDIO`, `BOM`, `EXCELENTE`) antes do texto livre. `<minVideo>` é um subconjunto de `<minMidia>` — só o tempo com uma aba do YouTube tocando **e** em foco (ver `YouTubeTabDetector.cs`). O Core desenha os oito números como oito linhas separadas, não como frase — é a única notificação com olhos pequenos **no topo** em vez de à esquerda. |
...
- **`REPORT`** (o Relatório do dia do Brobot.Sender, ver `DailyReportTracker.cs`
  ...
  precisa da tela inteira **abaixo** dos olhos para oito linhas empilhadas,
  uma por item, em vez de uma frase só. Chega numa linha própria, atômica
  como `ACHIEVEMENT`, mas com **sete números** antes do texto:
  `REPORT <buildOk> <buildFail> <commits> <minutosReuniao> <minutosMidia>
  <minutosVideo> <minutosJogo> <RATING> <texto>`. `<commits>` vem do hook de
  git que roda em `post-commit` (`hooks/mimo-git-hook.ps1`/
  `hooks/git-hooks/post-commit`), não de um monitor do Windows como os
  outros números. `<minutosVideo>` é um subconjunto de `<minutosMidia>` —
  não uma atividade separada — contando só o tempo com uma aba do YouTube
  tocando **e** em foco na frente do usuário (ver `YouTubeTabDetector.cs`).
  `<RATING>` é um de `PESSIMO`, `RUIM`, `QUESTIONAVEL`, `MEDIO`, `BOM`,
  `EXCELENTE` — o veredito que `DailyReportScoring.cs` já calculou; o Core
  não julga o dia, só decide como o veredito **aparece** na tela. As oito
  linhas (`Builds OK: N`, `Builds Falha: N`, `Commits: N`, `Reuniao: <fmt>`,
  `Midia: <fmt>`, `Video: <fmt>`, `Jogo: <fmt>`, `Desempenho: <rótulo>`) são
  escritas de uma vez, sem efeito de máquina de escrever — são dado, não
  ...
  qualquer outra notificação, só que começando mais embaixo na tela —
  depois das oito linhas de números, não na posição fixa de sempre.
